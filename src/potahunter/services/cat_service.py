...
class CATService(QObject):
    ...
    RADIO_MODELS = {
        ...
        "Yaesu FT-DX10": {"protocol": "kenwood", "baud": 38400},  # FT-DX10 uses Kenwood-compatible ASCII protocol
    ...
    # Mode mappings from radio-specific codes to standard modes
    KENWOOD_MODES = {
    ...
    def detect_baud_rate(port: str, radio_model: str) -> Optional[int]:
        ...
        logger.info(f"Auto-detecting baud rate for {radio_model} on {port}")

        for baud_rate in baud_rates:
            try:
                logger.info(f"Trying {baud_rate} baud...")
                ser = serial.Serial(
                    port=port,
                    baudrate=baud_rate,
                ...
                # Set DTR/RTS based on radio manufacturer
                ...
                # Try to read frequency based on protocol
                ...
                if protocol == "kenwood":
                    ...
                    if response:
                        ...
                        if digits and len(digits) >= 8:
                            ser.close()
                            logger.info(f"Detected baud rate: {baud_rate}")
                            return baud_rate
                ...
                logger.debug(f"Baud rate {baud_rate} failed: {e}")
                continue
    ...
    def connect(self, port: str, radio_model: str, custom_baud: Optional[int] = None) -> bool:
        ...
        try:
            ...
            radio_config = self.RADIO_MODELS[radio_model]
            self.radio_model = radio_model
            self.protocol = radio_config["protocol"]
            baud_rate = custom_baud if custom_baud else radio_config["baud"]

            # Open serial port
            logger.info(f"Opening serial port {port} at {baud_rate} baud, protocol: {self.protocol}")
            try:
            ...
            if self.protocol == "yaesu" or "Yaesu" in self.radio_model or "FT-" in self.radio_model:
                ...
                logger.info(f"Set DTR=False, RTS=False for Yaesu radio")
            else:
                # Kenwood and Icom typically use DTR/RTS on
                self.serial_port.dtr = True
                self.serial_port.rts = True
                logger.info(f"Set DTR=True, RTS=True for Kenwood/Icom radio")

            # Test connection by reading frequency (removed blocking sleep)
            logger.info(f"Testing connection by reading frequency...")
            freq = self.get_frequency()
            logger.info(f"Frequency response: {freq}")
            if freq is not None and freq > 0:
                self.is_connected = True
                self.current_frequency = freq
                self.connection_status_changed.emit(True)
                logger.info(f"Connected to {radio_model} on {port} at {baud_rate} baud")
    ...
    def get_frequency(self) -> Optional[float]:
        ...
        """
        logger.info(f"Is connected: {self.is_connected}, Serial port: {self.serial_port}")
        if not self.serial_port:
            return None
        logger.info("Getting frequency...")
        try:
            if self.protocol == "kenwood":
                return self._get_frequency_kenwood()
            elif self.protocol == "yaesu":
    ...
    def set_mode(self, mode: str) -> bool:
        ...
        Args:
            mode: Mode string (USB, LSB, CW, FM, AM, etc.)
        ...
        try:
            if self.protocol == "kenwood":
                return self._set_mode_kenwood(mode)
            elif self.protocol == "yaesu":
                return self._set_mode_yaesu(mode)
            elif self.protocol == "icom":
                return self._set_mode_icom(mode)
        except Exception as e:
    ...
    def _get_frequency_kenwood(self) -> Optional[float]:
        ...
        logger.info("Kenwood: Getting frequency")
        try:
            # Flush input buffer
            logger.info("Kenwood: Flushing input buffer")
            self.serial_port.reset_input_buffer()

            # Send FA; command (VFO-A frequency read) - preferred for Yaesu radios
            logger.info("Kenwood: Sending FA; command")
            ...
            try:
                text = response.decode('ascii', errors='replace')
                logger.debug(f"Kenwood: Decoded text: {text}")
                logger.debug(f"Kenwood: Response length: {len(text)}, format: {repr(text)}")

                # Extract all digits from response
                digits = ''.join(ch for ch in text if ch.isdigit())
                logger.info(f"Kenwood: Extracted digits: '{digits}' (length: {len(digits)} digits)")
                ...
                if digits and len(digits) >= 8:  # Need at least 8 digits for valid frequency
                    ...
                    logger.info(f"Kenwood: Parsed frequency: {freq_100hz} (100 Hz units) = {freq} Hz = {freq_mhz:.6f} MHz")
                    return freq
                else:
    ...
    def _set_frequency_kenwood(self, frequency: float) -> bool:
        ...
        try:
            ...
            logger.info(f"Kenwood: Setting frequency to {frequency} Hz ({frequency/1_000_000:.6f} MHz)")
            logger.info(f"Kenwood: Converted to integer: {freq_int}")
            logger.info(f"Kenwood: Converted to 100 Hz units: {freq_100hz}")
            logger.info(f"Kenwood: Formatted as 9-digit string: {freq_str}")
            logger.info(f"Kenwood: Final command: {command}")
            ...
            logger.info(f"Kenwood: Frequency command sent successfully")
            ...
            response = self.serial_port.read(128)
            if response:
                logger.info(f"Kenwood: Response after set frequency: {response}")
                ...
                logger.info(f"Kenwood: No response from radio after set frequency")

            # Verify the frequency was set by reading it back
            time.sleep(0.05)
            actual_freq = self._get_frequency_kenwood()
            if actual_freq:
                logger.info(f"Kenwood: Verification read shows frequency: {actual_freq} Hz")
                # Check if it's close (within 100 Hz tolerance for rounding)
                if abs(actual_freq - frequency) < 100:
                    logger.info(f"Kenwood: Frequency successfully set and verified!")
                    return True
                else:
                    logger.warning(f"Kenwood: Frequency mismatch! Wanted {frequency} Hz, got {actual_freq} Hz")
                    logger.warning(f"Kenwood: Radio may have frequency lock, band edges, or other restrictions enabled")
                    return False
            else:
    ...
    def _get_mode_kenwood(self) -> Optional[str]:
        """Get mode using Kenwood protocol"""
        self.serial_port.write(b"IF;")
        response = self.serial_port.read(38)
        if response and len(response) >= 38:
            mode_code = chr(response[29])
            return self.KENWOOD_MODES.get(mode_code, "UNKNOWN")
        return None

    def _set_mode_kenwood(self, mode: str) -> bool:
        """Set mode using Kenwood protocol"""
        mode_map = {v: k for k, v in self.KENWOOD_MODES.items()}
